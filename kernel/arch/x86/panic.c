...
static void _log_segs_ctrl(const segs_ctrl_t *sc) {
    ...
    log_fatal("  DS=%#06" PRIx16 " ES=%#06" PRIx16 " FS=%#06" PRIx16 " GS=%#06" PRIx16, sc->ds, sc->es, sc->fs, sc->gs);
    log_fatal(
        "  CR0=%#018" PRIx64 " CR2=%#018" PRIx64 " CR3=%#018" PRIx64 " CR4=%#018" PRIx64,
        sc->cr0,
        sc->cr2,
        sc->cr3,
        sc->cr4
...
static void _log_segs_ctrl(const segs_ctrl_t *sc) {
    ...
    log_fatal("  DS=%#06" PRIx16 " ES=%#06" PRIx16 " FS=%#06" PRIx16 " GS=%#06" PRIx16, sc->ds, sc->es, sc->fs, sc->gs);
    log_fatal(
        "  CR0=%#010" PRIx32 " CR2=%#010" PRIx32 " CR3=%#010" PRIx32 " CR4=%#010" PRIx32,
        (unsigned int)sc->cr0,
        (unsigned int)sc->cr2,
        (unsigned int)sc->cr3,
        (unsigned int)sc->cr4
...
static bool _frame_is_user(const arch_int_state_t *state) {
...
static void _dump_regs_from_state(const arch_int_state_t *state) {
    ...
    log_fatal(
        "  RIP=%#018" PRIx64 " RSP=%#018" PRIx64 " RFLAGS=%#018" PRIx64 " CS=%#06" PRIx64 " SS=%#06" PRIx64,
        state->s_regs.rip,
        stack_ptr,
        state->s_regs.rflags,
        state->s_regs.cs,
    ...
    log_fatal(
        "  RAX=%#018" PRIx64 " RBX=%#018" PRIx64 " RCX=%#018" PRIx64 " RDX=%#018" PRIx64,
        state->g_regs.rax,
        state->g_regs.rbx,
        state->g_regs.rcx,
        state->g_regs.rdx
    ...
    log_fatal(
        "   R8=%#018" PRIx64 "  R9=%#018" PRIx64 " R10=%#018" PRIx64 " R11=%#018" PRIx64,
        state->g_regs.r8,
        state->g_regs.r9,
        state->g_regs.r10,
        state->g_regs.r11
    ...
    log_fatal(
        "  R12=%#018" PRIx64 " R13=%#018" PRIx64 " R14=%#018" PRIx64 " R15=%#018" PRIx64,
        state->g_regs.r12,
        state->g_regs.r13,
        state->g_regs.r14,
        state->g_regs.r15
    ...
    log_fatal("register dump");
    log_fatal(
        "  EIP=%#010" PRIx32 " ESP=%#010" PRIx32 " EFLAGS=%#010" PRIx32 " CS=%#06" PRIx32 " SS=%#06" PRIx32,
        (unsigned int)state->s_regs.eip,
        (unsigned int)stack_ptr,
        (unsigned int)state->s_regs.eflags,
        (unsigned int)state->s_regs.cs,
        (unsigned int)stack_seg
    );
    log_fatal(
    ...
    log_fatal(
        "  EAX=%#010" PRIx32 " EBX=%#010" PRIx32 " ECX=%#010" PRIx32 " EDX=%#010" PRIx32,
...
static void _capture_regs(reg_snapshot_t *out) {
    ...
    __asm__ volatile("mov %%rax, %c[rax](%0)\n\t"
                     "mov %%rbx, %c[rbx](%0)\n\t"
                     "mov %%rcx, %c[rcx](%0)\n\t"
                     "mov %%rdx, %c[rdx](%0)\n\t"
                     "mov %%rsi, %c[rsi](%0)\n\t"
...
static void _dump_regs_snapshot(void) {
    ...
    log_fatal("  RSI=%#018" PRIx64 " RDI=%#018" PRIx64 " RBP=%#018" PRIx64, regs.rsi, regs.rdi, regs.rbp);
    log_fatal(
        "  RAX=%#018" PRIx64 " RBX=%#018" PRIx64 " RCX=%#018" PRIx64 " RDX=%#018" PRIx64,
        regs.rax,
        regs.rbx,
...
static void _capture_regs(reg_snapshot_t *out) {
    ...
    __asm__ volatile("mov %%eax, %c[eax](%0)\n\t"
                     "mov %%ebx, %c[ebx](%0)\n\t"
                     "mov %%ecx, %c[ecx](%0)\n\t"
                     "mov %%edx, %c[edx](%0)\n\t"
                     "mov %%esi, %c[esi](%0)\n\t"
