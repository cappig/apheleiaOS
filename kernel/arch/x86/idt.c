...
typedef struct {
    idt_register_t reg;
    int_handler_t handlers[ISR_COUNT];
    idt_entry_t entries[ISR_COUNT] ALIGNED(0x10);
} idt_state_t;
...
void configure_int(size_t int_num, u16 selector, u8 ist, u8 attribs) {
    ...
    idt_entry_t *descriptor = &idt.entries[int_num];
...
void idt_init(void) {
    ...
    for (size_t entry = 0; entry < ISR_COUNT; entry++) {
        idt_entry_t *descriptor = &idt.entries[entry];
    ...
    idt.reg.limit = ISR_COUNT * sizeof(idt_entry_t) - 1;
#if defined(__x86_64__)
    idt.reg.base = (u64)(uintptr_t)idt.entries;
#else
    idt.reg.base = (u32)(uintptr_t)idt.entries;
#endif
...
void idt_load(void) {
    asm volatile("lidt %0" : : "m"(idt.reg) : "memory");
}
...
void isr_handler(int_state_t *state) {
    ...
    if (state->int_num < ISR_COUNT && idt.handlers[state->int_num]) {
        idt.handlers[state->int_num](state);
        return;
    }
