...
static inline size_t utf8_decode(const u8 *data, size_t len, u32 *out) {
    ...
    if ((b0 & 0xf8) == 0xf0) {
        bool have_tail = len >= 4;
        bool cont1 = have_tail && (data[1] & 0xc0) == 0x80;
        bool cont2 = have_tail && (data[2] & 0xc0) == 0x80;
        bool cont3 = have_tail && (data[3] & 0xc0) == 0x80;

        if (!cont1 || !cont2 || !cont3) {
            return 0;
        }

        u32 cp = ((u32)(b0 & 0x07) << 18) | ((u32)(data[1] & 0x3f) << 12) | ((u32)(data[2] & 0x3f) << 6) |
                 (u32)(data[3] & 0x3f);
