...
void draw_fill_rect(u32* fb, u32 fb_width, u32 fb_height, i32 x, i32 y, u32 width, u32 height, u32 color) {
    if (!fb || !width || !height)
        return;
    ...
    for (i32 row = y0; row < y1; row++) {
        u32* dst = fb + (size_t)row * fb_width + (size_t)x0;
        for (i32 col = x0; col < x1; col++)
            *dst++ = color;
    }
}

void draw_fill_triangle(u32* fb, u32 fb_width, u32 fb_height, draw_point_t p0, draw_point_t p1, draw_point_t p2, u32 color) {
...
void draw_fill_polygon(u32* fb, u32 fb_width, u32 fb_height, const draw_point_t* points, size_t count, u32 color) {
    if (!fb || !points || count < 3)
        return;

    draw_point_t first = points[0];
    for (size_t i = 1; i + 1 < count; i++)
        draw_fill_triangle(fb, fb_width, fb_height, first, points[i], points[i + 1], color);
}
